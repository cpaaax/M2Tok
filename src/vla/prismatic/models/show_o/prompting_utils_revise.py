...
class UniversalPrompting():
    def __init__(self, text_tokenizer,
                 special_tokens=( "<soi>", "<eoi>", "<sot>", "<eot>"),
        ...
        self.text_tokenizer.add_special_tokens({'additional_special_tokens': list(special_tokens)})
        self.text_tokenizer.soi_token_id = self.text_tokenizer.convert_tokens_to_ids('<soi>')
        self.text_tokenizer.eoi_token_id = self.text_tokenizer.convert_tokens_to_ids('<eoi>')
        self.text_tokenizer.sot_token_id = self.text_tokenizer.convert_tokens_to_ids('<sot>')
        self.text_tokenizer.eot_token_id = self.text_tokenizer.convert_tokens_to_ids('<eot>')
    ...
    def mmu_prompt(self, image_ids, text_ids):
        ...
        for i in range(len(text_ids)):
            ...
            #     + 3是指下面的'<|mmu|>'+ '<|soi|>' + '<|eoi|>', 但我们不需要task token
            ...
            if max_text_len >= len(temp_ids):
                ...
                temp_ids = temp_ids + [self.pad_id] * (max_text_len - len(temp_ids))
                #
                temp_masks = [1] * (len(temp_ids) + image_ids.shape[-1] + 2) + [0] * (max_text_len - len(temp_ids))
            else:
                # should add the eos token
                temp_ids = temp_ids[:max_text_len - 1] + [self.text_tokenizer.eos_token_id]
                temp_masks = [1] * (len(temp_ids) + image_ids.shape[-1] + 2)  # +2 for two special tokens

            # 这里需要把text id后面紧跟着image id, 而不是image id + padded text id
            # prompting -- [task token] [sot] [text tokens] [eot] [soi] [image tokens] [eoi]
            temp_label_ids = torch.cat([
