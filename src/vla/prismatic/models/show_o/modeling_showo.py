    ...
    def __init__(
        ...
        if self.w_clip_vit:
            self.mm_projector = torch.nn.Sequential(
                ...
                torch.nn.GELU(),
                torch.nn.Linear(2048, 896)  # qwen-2.5-0.5B的hidden size是896
            )
            # self.mm_projector = torch.nn.Sequential(
            #
            #     torch.nn.Linear(1024, 2048),
            #     torch.nn.GELU(),
            #     torch.nn.Linear(2048, 2048)  # qwen-2.5-0.5B的hidden size是896
            # )
