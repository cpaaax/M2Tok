...
class TokenFlow(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        elif self.teacher == 'siglip_224':
            encoder_config["embed_dim"] = decoder_config["embed_dim"] = 1152
            img_size = 224
        ...
        if self.teacher == 'clipb_224':
            self.encoder_vqkd = clip.load_clip_vision("/cpfs01/user/xuchunpu/hf_models/ViT-B-16/ViT-B-16.pt", download_root='./clip_model')
        elif self.teacher == 'siglip_224':
            self.encoder_vqkd = SiglipVisionModel.from_pretrained("/cpfs01/user/xuchunpu/hf_models/siglip-so400m-patch14-224")
            ...
            self.encoder_vqkd = SiglipVisionModel.from_pretrained("/cpfs01/user/xuchunpu/hf_models/siglip-so400m-patch14-384")
            # Freeze the head's parameters and 26th layer's parameters
            for name, param in self.encoder_vqkd.named_parameters():
                if 'head' in name:
                    param.requires_grad = False
        ...
        if self.teacher == 'clipb_224':
            ...
            self.scaling_layer = ScalingLayerForSigLip() # can be changed to ScalingLayerForClip()
        elif self.teacher == 'vitamin_xlarge_256':
    ...
    def encode(self, x):
        ...
        if self.teacher == "clipb_224":
            ...
            vqkd_x = self.scaling_layer(x)  # x:[batch,3,224,224]
            vqkd_feature = self.encoder_vqkd(vqkd_x, output_hidden_states=True).hidden_states[-2]  # [batch, 256, 1152]
            vqkd_feature = self.encode_task_layer(vqkd_feature.type_as(self.encode_task_layer[-1].weight)) # [batch, 256, 32]
            N = vqkd_feature.shape[1]  # 256
            h, w = int(math.sqrt(N)), int(math.sqrt(N))  # 16,16
            vqkd_feature = rearrange(vqkd_feature, 'b (h w) c -> b c h w', h=h, w=w)  # reshape for quantizer  # [batch, 32, 16, 16]
        elif self.teacher == 'vitamin_xlarge_256':
            ...
            vqkd_x = self.scaling_layer(x)  # x:[batch,3,384,384]
            vqkd_feature = self.encoder_vqkd(vqkd_x, output_hidden_states=True).hidden_states[-2]  # [batch, 729, 1152]
            vqkd_feature = self.encode_task_layer(vqkd_feature.type_as(self.encode_task_layer[-1].weight))   # [batch, 729, 32]
            N = vqkd_feature.shape[1]  # 729
            h, w = int(math.sqrt(N)), int(math.sqrt(N))
            vqkd_feature = rearrange(vqkd_feature, 'b (h w) c -> b c h w', h=h, w=w) # reshape for quantizer  # [batch, 32, 27, 27]
        else:
            raise ValueError(f'teacher {self.teacher} not supported')

        # vqgan
        if self.teacher in ['clipb_224', 'vitamin_xlarge_256']:
            vqgan_features = self.encoder(x)    # [batch, 256, 14, 14]
            vqgan_features = self.quant_conv(vqgan_features)  # [batch, 8, 14, 14]
        elif self.teacher in ['siglip_224', 'siglip_384']:
            upscale_reso = 2**4 * vqkd_feature.shape[-1] # 256 for siglip_224, 432 for siglip_384
            vqgan_x = F.interpolate(x, size=(upscale_reso, upscale_reso), mode='bicubic') # [batch, 3, 256, 256] for siglip_224, [batch, 3, 432, 432] for siglip_384
            # vqgan_x = x
            vqgan_features = self.encoder(vqgan_x)  # [batch, 256, 16, 16] for siglip_224, [batch, 432, 27, 27] for siglip_384
            vqgan_features = self.quant_conv(vqgan_features)  # [batch, 8, 16, 16] for siglip_224, [batch, 8, 27, 27] for siglip_384
        else:
            raise ValueError(f'teacher {self.teacher} not supported')
        # for siglip_224: vqkd_feature: [batch, 32, 16, 16], vqgan_features: [batch, 4, 16, 16]
        # for siglip_384: vqkd_feature: [batch, 32, 27, 27], vqgan_features: [batch, 4, 27, 27]
    ...
    def decode(self, quant):
        ...
        if self.teacher == 'siglip_384':
            ...
            vqgan_recon = F.interpolate(vqgan_recon, size=(224, 224), mode='bicubic')
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        z = torch.einsum('b c h w -> b h w c', z).contiguous()  # [1,40,27,27]->[1,27,27,40]
        z_flattened = z.view(-1, self.e_dim) # [batch*27**2, 40]
        ...
        if self.l2_norm:
            z_flattened_vqkd = F.normalize(z_flattened_vqkd, p=2, dim=-1)  # [batch*27**2, 32]
            z_flattened_vqgan = F.normalize(z_flattened_vqgan, p=2, dim=-1)  # [batch*27**2, 8]
            z_flattened = torch.cat([z_flattened_vqkd, z_flattened_vqgan], dim=-1)  # [batch*27**2, 40]
        ...
        if self.l2_norm:
            ...
            embedding_vqkd = F.normalize(self.embedding_vqkd.weight, p=2, dim=-1)  # [32768, 32]

            z_vqgan = F.normalize(z_vqgan, p=2, dim=-1)
            embedding_vqgan = F.normalize(self.embedding_vqgan.weight, p=2, dim=-1)  # [32768, 8]

            z = torch.cat([z_vqkd, z_vqgan], dim=-1) # [batch, 27,27,40]

        else:
            embedding_vqkd = self.embedding.weight[:, :self.split[0]]
            embedding_vqgan = self.embedding.weight[:, self.split[0]:]

        a = torch.sum(z_flattened_vqkd ** 2, dim=1, keepdim=True)  # [batch*27**2,1]
        b = torch.sum(embedding_vqkd**2, dim=1)  # [32768]
        c = torch.einsum('bd,dn->bn', z_flattened_vqkd, torch.einsum('n d -> d n', embedding_vqkd)) # [batch*27**2,32768]

        # 欧式距离, (x-e)^2=x^2+e^2-2x*e
        d_vqkd = torch.sum(z_flattened_vqkd ** 2, dim=1, keepdim=True) + \
                torch.sum(embedding_vqkd**2, dim=1) - 2 * \
                torch.einsum('bd,dn->bn', z_flattened_vqkd, torch.einsum('n d -> d n', embedding_vqkd))  # 计算距离，[batch*27**2,32768]
        ...
        # torch.unique of BFfloat16 is not supported for torch2.5.0 and torch2.4.0, it might be figured out later
        ...
        z_q = z + (z_q - z).detach()
        # print(z_q.squeeze())
        # print(all_embedding[min_encoding_indices[0]])
        # reshape back to match original input shape
        z_q = torch.einsum('b h w c -> b c h w', z_q)  # z_q是min_index对应的embedding
