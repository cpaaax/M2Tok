...
class VQLoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
                 ...
                 codebook_weight=1.0, perceptual_weight=1.0, teacher="clipb_224", vgg16_pretrained_path=None
    ):
        super().__init__()
        # discriminator loss
        ...
        # perceptual loss
        self.perceptual_loss = LPIPS(vgg16_pretrained_path=vgg16_pretrained_path).eval()
        ...
        if self.teacher in ['clipb_224', "siglip_224", 'vitamin_xlarge_256']:
        ...
        if self.teacher == 'clipb_224':
            ...
            print(f"Use siglip-so400m-patch14-224 as teacher model.")
            self.teacher_model = SiglipVisionModel.from_pretrained("/cpfs01/user/xuchunpu/hf_models/siglip-so400m-patch14-224")
        elif self.teacher == 'siglip_384':
            print(f"Use siglip-so400m-patch14-384 as teacher model.")
            self.teacher_model = SiglipVisionModel.from_pretrained("/cpfs01/user/xuchunpu/hf_models/siglip-so400m-patch14-384")
        else:
