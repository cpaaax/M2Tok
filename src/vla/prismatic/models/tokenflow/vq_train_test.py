...
def main(args):
    ...
    vq_loss = VQLoss(
        ...
        teacher=args.teacher,
        vgg16_pretrained_path=args.vgg16_pretrained_path
    ).to(device)
    ...
    if args.vq_ckpt:
        ...
        if args.finetune_decoder and args.enhanced_decoder:
            ...
            logger.info(f"Loaded model from checkpoint {args.vq_ckpt}.")
        ...
        try:
            ...
            logger.info(f"Loaded Optimizer from checkpoint {args.vq_ckpt}.")
        except:
            logger.info("Optimizer starting from scratch.")
        try:
            vq_loss.discriminator.load_state_dict(checkpoint["discriminator"])
            logger.info(f"Loaded discriminator from checkpoint {args.vq_ckpt}.")
        except:
            logger.info("Discriminator starting from scratch.")
        try:
            optimizer_disc.load_state_dict(checkpoint["optimizer_disc"])
            logger.info(f"Loaded optimizer_disc from checkpoint {args.vq_ckpt}.")
        except:
...
if __name__ == "__main__":
    ...
    # TODO
    parser.add_argument("--data-path", type=str, default='/cpfs01/user/xuchunpu/embodied_codes/TokenFlow-main_mine/tokenflow/get_dataset_info') # 在这个目录下，提供image_paths.json
    ...
    # TODO
    parser.add_argument("--teacher", type=str, choices=["clipb_224", "vitamin_xlarge_256", "siglip_384", "siglip_224"], default="siglip_224", help="the semantic teacher, important")

    parser.add_argument("--vgg16_pretrained_path", type=str, default="/cpfs01/user/xuchunpu/hf_models/vgg16/vgg16-397923af.pth", help="ckpt path for resume training")
    parser.add_argument("--vq-ckpt", type=str, default=None, help="ckpt path for resume training")  # "/cpfs01/user/xuchunpu/embodied_codes/TokenFlow-main_mine/results_tokenizer_image/029-TokenFlow/checkpoints/0015000.pt"
    ...
    parser.add_argument("--results-dir", type=str, default="/oss-xuchunpu/train_tokenflow_weights/results_tokenizer_image")
    ...
    parser.add_argument("--mixed-precision", type=str, default='bf16', choices=["none", "fp16", "bf16"]) # better change to bf16 if GPU support
    ...
    parser.add_argument("--enhanced_decoder", default=True, help="whether using enhanced decoder")
