...
class TokenFlow(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.quantize = VectorQuantizer(config.codebook_size, self.code_dim,
                                        ...
                                        split=[self.semantic_code_dim, self.vqgan_code_dim], kmeans=config.kmeans)
        ...
        if self.using_shared_conv:
            quant_resi = 0.5
            share_quant_resi = 4
            self.vqkd_quant_resi = PhiPartiallyShared(nn.ModuleList(
                [(Phi(self.vqkd_code_dim, quant_resi) if abs(quant_resi) > 1e-6 else nn.Identity()) for _ in
                 range(share_quant_resi)]))
            self.vqgan_quant_resi = PhiPartiallyShared(nn.ModuleList(
                [(Phi(self.vqgan_code_dim, quant_resi) if abs(quant_resi) > 1e-6 else nn.Identity()) for _ in
                 range(share_quant_resi)]))
    ...
    def encode(self, x):
        ...
        if self.enable_var:
            ...
            for i, scale_size in enumerate(new_layers):  # TODO add random drop
                ...
                quantize = F.interpolate(quantize, size=(self.scale_rq_layers[-1], self.scale_rq_layers[-1]),
                                         mode='bicubic')

                if self.using_shared_conv:  # default not use
                    quantize_vqkd, quantize_vqgan = torch.split(quantize,
                                                                split_size_or_sections=[self.semantic_code_dim,
                                                                                        self.vqgan_code_dim], dim=1)
    ...
    def forward(self, z):
        ...
        if self.kmeans_init and not self.initted:  # default not use kmeans
            with torch.no_grad():
                ...
                z_flattened_vqkds = [torch.zeros_like(z_flattened_vqkd) for _ in
        ...
        if aggregate_usage:
            with torch.no_grad():
                min_encoding_indices_all = [torch.zeros_like(min_encoding_indices) for _ in
                                            range(torch.distributed.get_world_size())]
        ...
        if self.training:
            if self.kmeans_init:
                ...
                commit_loss = self.beta * torch.mean((z_q_vqkd.detach() - z_vqkd) ** 2) + self.beta * torch.mean(
                    (z_q_vqgan.detach() - z_vqgan) ** 2)
        ...
        return z_q, (vq_loss, commit_loss, entropy_loss, codebook_usage, vqkd_d_norm, vqgan_d_norm), (
