...
class VectorQuantizer(nn.Module):
    ...
    def f_to_idx(self, features):
        ...
        return indices.view(B, L), features_hat.view(B, L, C)
...
class VectorQuantizerM(nn.Module):
    ...
    def f_to_idx(self, features):
        indices, feature_hats = [], []
        chunk_size = features.shape[-1] // self.num_codebooks
        splited_features = features.split(chunk_size, dim=-1)
        for i, codebook in enumerate(self.codebooks):
            index, feature_hat = codebook.f_to_idx(splited_features[i])
            indices.append(index+i*(self.vocab_size // self.num_codebooks))
            feature_hats.append(feature_hat)
        indices = torch.stack(indices, dim=2)  # [batch, chunk_size/4, head_num]
        feature_hats = torch.stack(feature_hats, dim=2) #  # [batch, chunk_size/4, head_num, 256]
        return indices, feature_hats

    def idx_to_specifical_codebook_f(self, idx, indices):  # 这里indices对应idx-th codebook
        assert idx <= self.num_codebooks-1

        cur_codebook = self.codebooks[idx]
        latent_feature = cur_codebook.codebook(indices-idx*(self.vocab_size // self.num_codebooks))  # 因为每个codebook的vocab都是self.vocab_size // self.num_codebooks
        return latent_feature


    def idx_to_f(self, indices):  # 这里indices是完整的一组action index
        assert indices.shape[2] == self.num_codebooks
        latent_features = []
        for i, codebook in enumerate(self.codebooks):
            sub_indices = indices[:, :, i]

            # print(f"sub_indices is {sub_indices}, i is {i}, {sub_indices-i*(self.vocab_size // self.num_codebooks)}")

            latent_feature = codebook.codebook(sub_indices-i*(self.vocab_size // self.num_codebooks))
